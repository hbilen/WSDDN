function vl_compilenn_wsddn2(varargin)
...
root = [pwd filesep 'matconvnet'];
addpath(fullfile(root, 'matlab')) ;

layersPath = [pwd filesep 'layers'];
addpath(layersPath) ;
...
lib_src{end+1} = fullfile(layersPath,'matlab','src',['nnspp.' ext]) ;
...
mex_src{end+1} = fullfile(layersPath,'matlab','src',['vl_nnspp.' ext]) ;
...
lib_src{end+1} = fullfile(layersPath,'matlab','src','spp_cpu.cpp') ;
...
if opts.enableGpu
  ...
  lib_src{end+1} = fullfile(layersPath,'matlab','src','spp_gpu.cu') ;
end
...
flags.cc = {} ;
flags.link = {} ;
flags.cc{end+1} = ['-I' fullfile(root,filesep,'matlab',filesep,'src')];
